...
`shared/map.js` exposes map generation and routing. `shared/movement.js` contains shared geometry, collision resolution, sight queries, and visibility polygons. `shared/simulation.js` is the public simulation facade and explicit fixed-tick coordinator. `server/index.js` owns rooms, connections, recording, archives, and static asset delivery. `public/client.js` owns application startup, networking, authoritative state, prediction/reconciliation, lobby flow, and replay lifecycle; `public/arena-scene.js` renders the world and visibility mask.
