...
Shutdown is an explicitly owned asynchronous operation. Logical match completion and archive publication are separate: the room service tracks outstanding finalizations even after a room is retired, and `close()` awaits them. Repeated writer/service/server close calls share completion. Server shutdown stops admission and timers before awaiting storage. An archive failure reports an error rather than a saved replay and still permits shutdown to settle. This corrects the previous shutdown race in a separate reliability checkpoint; it does not decide whether a live game should continue after recording fails.
...
`shared/simulation/projection-contract.js` explicitly lists the live frame, own-player, equipment, gate, trap, projectile, effect and event fields. Source-key order and optional-field presence are retained for existing serialized output. Adding simulation bookkeeping now requires deliberate opt-in before it appears in those payloads. Other-player fields retain their existing narrower contract; snapshots and recordings remain complete. Static welcome-map content still follows the prototype map contract, and permitted fields retain their current types: these lists are not a protocol migration framework or full schema validator.
