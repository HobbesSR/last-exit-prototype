...
present for checkpoint context, then the relevant sections of `REQUIREMENTS.md`,
`ENCAPSULATION_PLAN.md` and `ARCHITECTURE.md`. Requirements and newer user edits take
precedence over stale handoff summaries. Preserve `FRESH_SESSION.md` and its earlier
notes; it must remain untracked. Do not stage or overwrite unrelated user changes.
...
See `DELEGATION.md` for the task brief and manual-switch handoff convention.
...
- Read current requirements before implementing accepted gameplay additions.
  ...
  existing user answers or explicitly recorded assumptions appropriate to the task.
