...
These and the September 11 additions are tracked as F-01 through F-18 in `REQUIREMENTS.md`. The accepted next direction includes three hunters, weapon quality tiers, drag-and-drop weapons, reloads preserving unused loaded ammo, ammunition pickups and forces/impulses/friction. Weapon quality and trap/hazard density and danger should increase with vertical excursion and rightward progress. Dormant traps give subtle tells, become overt on proximity activation and conceal themselves again after deactivation; rearming is a separate decision. Opening-based interior/exterior sight should follow the user's described proximity rule. These additions are not yet implemented. The requested hierarchical template system is explicitly deferred; the interim maze improves playability without claiming to implement it. Indoor/outdoor mechanics are implemented, but 2½D presentation and finished art remain incomplete. Batched design questions are in requirements section 8.

Inventory currently uses six icon-based slots, finite ammo, move/swap/merge controls and retained ammunition on drops. Weapon pickups replenish matching ammunition. Reloads and ammo pickups are accepted requirements; magazine/reserve details and tier compatibility remain open. Current ammo is attached to each weapon.
