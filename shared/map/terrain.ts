...
export function buildTerrain(context: GenerationContext): void {
  ...
  for (const n of map.nodes) for (const [dx, dy] of [[1, 0], [-1, 0], [0, 1], [0, -1]]) {
    const q = lookup.get((n.col + dx) + ',' + (n.row + dy) as NodeId);
  ...
  for (const n of map.nodes) {
    ...
    const q = lookup.get((n.col + 1) + ',' + n.row as NodeId);
