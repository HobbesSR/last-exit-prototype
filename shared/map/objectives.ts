...
import { WORLD_WIDTH } from './world.ts';
import { distance } from './context.ts';
import type { SpawnedGenerationContext } from './context.ts';
import type { ChargerId, ItemKind, SensorId, StationId, TrapId, TrapKind, WeaponType } from '../types.ts';

/** Places starter weapons, transit stations, sensors, chargers, loot and traps. */
export function buildObjectives(context: SpawnedGenerationContext): void {
  ...
  for (const [i, n] of main.entries()) if (i > 3 && i < main.length - 3 && i % 6 === 0) {
    const station = { id: nextId('rail-') as StationId, x: n.x, y: n.y, nodeId: n.id }; map.stations.push(station); occupied.push(station);
  }
  map.stations.sort((a, b) => a.x - b.x || a.y - b.y);
  for (const index of [10, 20, 30]) if (main[index]) { const n = main[index]; map.sensors.push({ id: nextId('sensor-') as SensorId, x: n.x + 80, y: n.y - 80 }); }
  ...
  for (const [i, n] of objectiveNodes.entries()) if (i % 9 === 0) {
    const charger = { id: nextId('charger-') as ChargerId, x: n.x - 130, y: n.y + 100, nodeId: n.id };
  ...
  for (const [i, spot] of spots.entries()) {
    ...
    const kind = (spot.buildingId ? (i % 3 === 0 ? 'cell' : 'weapon') : i % 7 === 0 ? 'cell' : ['weapon', 'med', 'shield', 'access'][range(0, 3)]) as ItemKind;
    place(spot.x, spot.y, kind, { ...(kind === 'weapon' ? { weaponType: ['pistol', 'rifle', 'scattergun'][range(0, 2)] as WeaponType } : {}), ...(spot.buildingId ? { buildingId: spot.buildingId } : {}) });
  }
  ...
  for (const [i, n] of map.nodes.entries()) if (i % 11 === 0 && distance(n, entryNode) > 2500) {
    ...
    const kind = ['mine', 'turret', 'flame', 'spider'][range(0, 3)] as TrapKind;
    map.traps.push({ id: nextId('trap-') as TrapId, kind, ...point, homeX: point.x, homeY: point.y, heading: -Math.PI / 2, offset: range(0, 159), cooldown: 35, spent: false }); occupied.push(point);
  }
}
